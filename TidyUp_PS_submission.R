...
tbl <- read.csv("IAT.csv") #I also set my working directory to the source file location
View(tbl)
...
tbl_clean <- tbl %>%
  select("session_id", "gender", "raceomb_002", "D_biep.White_Good_all", 
         "Mn_RT_all_3467", "edu_14", "politicalid_7", "STATE", "att_7", "tblacks_0to10", 
...
#identifying NA rows
na <- tbl_clean[which(is.na(tbl_clean$D_biep.White_Good_all)),] 
count_na <- dim(na)[1]
...
tbl_clean <- tbl_clean %>%
  rename(id = session_id, race = raceomb_002, bias = D_biep.White_Good_all, RT = Mn_RT_all_3467, edu = edu_14, pol = politicalid_7, state = STATE,
...
str(tbl_clean) #making sure this worked (it worked)
...
tbl_clean$gender4 <- recode(tbl_clean$gender, '[1]'='M', '[2]'='F', '[5]'='Q', .default = 'Other')
...
tbl_clean$edu7 <- recode(tbl_clean$edu7, '1' = 'no HS', '2'='no HS', '3'='some HS', 
                         '4'='graduated HS', '5'='some coll', '6'='postsec degree', 
                         '7' = 'postsec degree', '8'='some grad', '9'='MA/MBA', 
                         '10'='ADV degree', '11'='ADV dgree', '12'='ADV degree')
...
tbl_clean <- tbl_clean %>%
  mutate(gender4 = recode(gender, '[1]'='M', '[2]'='F', '[5]'='Q', .default = 'Other'),
         edu7 = recode(as.factor(edu), '1' = 'no HS', '2'='no HS', '3'='some HS', '4'='graduated HS', 
                       '5'='some coll', '6'='postsec degree', '7' = 'postsec degree', '8'='some grad', 
                       '9'='MA/MBA', '10'='ADV degree', '11'='ADV dgree', '12'='ADV degree'))
...
tbl_clean %>%
  filter(gender4 == 'M' & edu7 == 'ADV degree') %>%
  summarize(mean_bias_for_men_ADV = mean(bias))  

# high school graduates who are men
tbl_clean %>%
  filter(gender4 == 'M' & edu7 == 'graduated HS') %>%
